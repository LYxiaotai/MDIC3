...
if param["-grnchoose"] == 'GNIPLR':
    ...
    if "-process" not in param.keys() or "-step" not in param.keys():
        ...
        print(help_msg)
        exit()
    if '-grnsave' in param.keys():
        grnsave = param['-grnsave']
...
if __name__ == '__main__':
    ...
    if choose == 'GNIPLR':
        ...
        if grnsave == 'TRUE':
            print('Saving GNIPLR GRN results...')
    ...
    ccc_adjacency = MDIC3(AA, GRN)
    type_adjacency = celltype_score(labelsl, indexa, ccc_adjacency)
    ...
    np.savetxt(fold + os.sep + 'cellular_communication.txt', ccc_adjacency)
