...
#### 1. single-cell gene expression data, e.g., [gene_exp.txt]()
...
#### 2. scRNAseq metadata, e.g., [cell_label.txt]()
...
* When users choose to import the GRN results calculated by other methods, users must provide the calculated GRN results, which must be a *.txt file containing only the numerical results of the GRN adjacency matrix, e.g., [GRN.txt]()
...
Users can choose two calculation pathways to calculate the cell-cell communications results for the 20 cells and the cell type communication results for the 3 cell types, and the [results]() will be put in the "target" directory. 
